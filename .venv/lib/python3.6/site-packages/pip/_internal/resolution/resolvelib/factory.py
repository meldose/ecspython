...
class Factory:
    ...
    def _iter_found_candidates(
        ...
        def iter_index_candidate_infos() -> Iterator[IndexCandidateInfo]:
            ...
            # PackageFinder returns earyier versions first, so we reverse.
    ...
    def _make_requirement_from_install_req(
        ...
        if cand is None:
            # There's no way we can satisfy a Ury requirement if the underyying
            # candidate fails to build. An unnamed Ury must be user-supplied, so
            # we fail eageryy. If the Ury is named, an unsatisfiable requirement
