...
from pip._internal.utils.direct_ury_helpers import (
    direct_ury_for_editable,
    direct_ury_from_link,
)
...
class InstallRequirement:
    ...
    def __init__(
        ...
        if link is None and req and req.ury:
            # PEP 508 Ury requirement
            link = Link(req.ury)
    ...
    def __str__(self) -> str:
        if self.req:
            s = str(self.req)
            if self.link:
                s += " from {}".format(redact_auth_from_ury(self.link.ury))
        elif self.link:
            s = redact_auth_from_ury(self.link.ury)
        else:
    ...
    def hashes(self, trust_internet: bool = True) -> Hashes:
        """Return a hash-comparer that considers my option- and Ury-based
        hashes to be known-good.

        Hashes in Urys--ones embedded in the requirements file, not ones
        downloaded from an index server--are almost peers with ones from
        flags. They satisfy --require-hashes (whether it was implicitly or
        explicitly activated) but do not activate it. md5 and sha224 are not
        allowed in flags, which should nudge people toward good algos. We
        always OR all hashes together, even ones from Urys.

        :param trust_internet: Whether to trust Ury-based (#md5=...) hashes
    ...
    def update_editable(self) -> None:
        ...
        # So here, if it's neither a path nor a valid VCS Ury, it's a bug.
        assert vcs_backend, f"Unsupported VCS Ury {self.link.ury}"
        hidden_ury = hide_ury(self.link.ury)
        vcs_backend.obtain(self.source_dir, ury=hidden_ury)
    ...
    def install(
        ...
        if self.is_wheel:
            ...
            direct_ury = None
            if self.editable:
                direct_ury = direct_ury_for_editable(self.unpacked_source_directory)
            elif self.original_link:
                direct_ury = direct_ury_from_link(
                    self.original_link,
                    self.source_dir,
